...
// 任意类型
const anyArray: Array<any> = ['张三', 18, {}]
```
...
padLeft('Hello world', '张三 ') // '张三 Hello world'
...
const instance = new Man('李四')
console.log(instance) // Man { name: '李四' }
instance.sayHello() // 'hello, 李四'
```
...
const man = new Person('张三')
man.name = '李四' // Error: 无法为“name”赋值，因为它是只读属性
```
...
const instance = new Man('李四',  20, '浙江杭州')

console.log(instance.name)     // 李四
...
const instance = new Person('张三')
instance.name = '李四' // Error: 无法为“name”赋值，因为它是只读属性
```
...
console.log(identity<string>('张三')) // '张三'
```
